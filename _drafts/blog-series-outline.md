...
**The through-line tension:** The full circle from human intermediaries → self-service → AI intermediaries isn't just about convenience returning at lower cost. The convenience/conflict trade-off swings back too. Self-service won partly because it was *less conflicted* than human intermediaries (no commissions, no preferred partnerships — you saw the raw options). AI intermediaries reintroduce the conflict-of-interest problem (promoted placements, filtered information) in a potentially worse form, because the agent feels neutral in a way a human intermediary never did. The conversational format removes old manipulation vectors (UI-level dark patterns) but introduces new ones (information filtering in a trusted context). The real question is whether the net effect is genuinely better than today's status quo. This tension is introduced in Post 1 (economics sketch), explored from the consumer side in Post 2, examined from the builder side in Post 3, analyzed competitively in Post 4, and fully confronted in Post 5 (trust problem, equity question). Every post touches it; Post 5 resolves it (or rather, honestly admits it's unresolved). This is what elevates the series from "here's a prediction" to "here's the question that matters."
...
- Dark patterns reframed: the conversational format removes old manipulation vectors (hidden fees, buried buttons, urgency messaging) but introduces new ones (information filtering in a trusted context, promoted placements that feel like personal recommendations). The question is net effect vs. the status quo, not perfection.
...
Covers:
- The full circle in depth: human intermediaries → self-service (SaaS) → AI intermediaries — and the convenience/conflict trade-off that swings back with it. Self-service won not just on cost but because it was less conflicted (no commissions, you saw the raw options). AI intermediaries bring back convenience *and* the trust problem. Scoped honestly: closes for friction-heavy self-service, not for control-as-value.
- **Why would providers join? The supply-side argument.** The demand side (consumers want this) is developed in Post 2. But a platform with no services is a search engine with no websites. The thesis depends on providers participating — and many of them get commoditized in the process. So why would they? The answer is a prisoner's dilemma, not a willing embrace — and the pattern is familiar from every prior platform transition. (1) **The historical parallel:** Hotels didn't choose Booking.com. They joined for incremental bookings, then consumer behavior shifted, and opting out meant invisibility. Businesses didn't choose Google — they had to be where consumers looked. Developers didn't love the 30% App Store cut, but that's where the users were. The pattern: providers join because the alternative — being invisible while competitors are visible — is worse. First movers get distribution advantage; once critical mass forms, holdouts are punished. (2) **The positive case (underplayed but real):** MCP Apps mean providers aren't fully commoditized — they control their presentation, brand, and UX within the agent's interface. They're a website rendered in a browser they don't control, which is exactly what the web already is. The agent sends qualified, high-intent traffic — no browsing, no bouncing. And providers shed the cost of building and maintaining a full consumer-facing app; their "interface" becomes a display template. (3) **The honest breakdown by category:** Companies with real capability behind the UI benefit — they get distribution at lower cost. Companies whose only value *was* the UI get destroyed. Companies in the middle join reluctantly because the alternative is worse. That's exactly how hotels, merchants, and app developers experienced the last three platform transitions. This section earns the disruption framework that follows — without it, the dies/commoditized/untouched categories are asserted, not argued.
- The three independent forces driving the end of the SaaS playbook — post-ZIRP economics, LLM commoditization, agentic coding — which don't depend on the platform thesis being right
...
- The AI-enhanced app trap: why bolting *shallow* LLM features onto existing products is the most vulnerable strategy (three layers of vulnerability — general LLM wrappers vs. interface-level AI vs. genuinely new capabilities). Products with deep domain AI and proprietary data are in a different category — they become infrastructure the agent calls, not something it replaces.
...
- The VC capital thesis: capital as mechanism, not validation. VC money will flow because it needs deployment. If the consumer habit forms (conversational interaction becomes default like googling), the infrastructure capital builds becomes the platform. If not, the infrastructure persists as better tooling. What's investable: vertical agents, infra, picks-and-shovels, content aggregators.
...
Covers:
- Google's innovator's dilemma (75% of revenue from ads, can't cannibalize search). Google's MCP resistance is the innovator's dilemma in action — embedding Gemini into Search, Gmail, Docs is the "AI as feature layer" strategy, the rational bet of the company with the most to lose if a cross-cutting platform emerges. This is also the strongest version of the counterargument: if Google's strategy wins, no new platform gatekeeper emerges.
- Apple's structural advantage (devices + payments + identity + developer ecosystem) — BUT Apple hasn't executed on the model layer, partly due to privacy constraints on cloud-based inference. Structural advantage doesn't matter if you can't ship the product. The race is whether insurgents establish distribution and habits before Apple catches up.
- Multi-device handoff: earbuds handle conversation and low-stakes delegation, high-stakes confirmation flows to a device with screen and biometrics. The device ecosystem works together — which is Apple's natural strength IF they solve the agent layer.
- Three different bets on where the gatekeeper lives: Apple (device controls the agent), OpenAI (own the device via Jony Ive hire + messaging distribution via OpenClaw — hedging both layers), Anthropic (protocol is the platform via MCP Apps — no device layer, pure services bet). These can be evaluated on factors that exist today (distribution, model quality, developer ecosystem, consumer habits). Wearables are a possible accelerant that would intensify Apple's advantage if they materialize at consumer scale, but the analysis doesn't depend on them. **OpenAI's structural weakness (engage with Evans):** Ben Evans argues that OpenAI's platform stack looks ambitious but lacks the coercive power that made prior platform stacks work — no network effects, no lock-in, shallow engagement (5% of ChatGPT users pay, 80% sent fewer than 1,000 messages in 2025). Product strategy flows from research, not user needs (Fidji Simo: "I get a researcher pinging me saying: 'I have something pretty cool. How are you going to use it in chat?'" — the opposite of Jobs' "start with customer experience and work backwards"). Evans makes the case that execution alone, without structural advantages, is historically insufficient for sustained platform dominance. This is the strongest specific critique of OpenAI's position and should be engaged with directly — it sharpens why the OpenClaw distribution play matters so much (it's an attempt to manufacture the engagement depth OpenAI currently lacks).
- Centralized vs. decentralized: Claude/ChatGPT platforms vs. OpenClaw self-hosted agents — the hybrid future. **Reminder from Post 3:** Post 3 notes that the addressable market fragments by trust model — not everyone will hand health/financial data to a US platform, some will prefer OSS/regional agents. The demand doesn't disappear, it fragments. Develop this here: US consolidation push vs. European/privacy-driven preference for self-hosted or regionally controlled agents. Connect to the sovereignty argument.
...
- The strongest counterargument: what if the platform doesn't consolidate? (LLMs as distributed feature layer vs. cross-cutting platform). **Evans provides the best-articulated version of this argument**, with two specific challenges that deserve honest engagement:
  - *Engagement data as demand-side evidence.* Evans' ChatGPT usage data (800-900M users, 5% pay, 80% sent fewer than 1,000 messages in 2025) directly challenges whether the delegation habit forms. This is harder to dismiss than the generic "feature layer" counterargument because it's empirical — people *have* the tool and mostly don't use it deeply. US teens use chatbots "a few times a week or less," not daily. The honest response: current chatbot usage measures a conversational search tool, not a delegation agent — the habit we're predicting (delegate a task, review, confirm) doesn't exist yet in product form. But Evans' data is the hardest piece to dismiss because the tool exists, hundreds of millions have tried it, and deep engagement hasn't formed for most.
  - *The widget fallacy.* Evans argues that reducing complex products to simple API calls historically fails because providers resist ceding control of their user experience — developers don't want to become "dumb API calls." This directly challenges the MCP-as-HTTP analogy. The response: MCP Apps return rich UIs (providers keep control of presentation), so the parallel isn't "website reduced to API" but "website rendered inside a browser the provider doesn't control" — which is exactly what the web did. But the tension is real and deserves naming: providers will resist, and the degree to which MCP Apps preserve provider agency determines adoption speed.
- Reference: [Wardley Map analysis of AI platform wars](https://www.linkedin.com/pulse/ai-platform-wars-wardley-map-openai-vs-anthropic-google-mohandass-uffoc/) — reaches similar conclusions from an enterprise/CIO lens using Wardley mapping. Useful for concrete market share data (Anthropic enterprise share 12% → 40%, OpenAI 50% → 27%), financial dynamics (OpenAI's projected $14B losses vs Anthropic's path to profitability), the Palantir analogy for OpenAI's forward-deployed engineer strategy, and Microsoft as a cautionary tale of distribution without differentiation. Notably omits Apple entirely — which strengthens our point that Apple is the underexplored bet.
- Reference: [Ben Evans, "How Will OpenAI Compete?"](https://www.ben-evans.com/benedictevans/2026/2/19/how-will-openai-compete-nkg2x) (2026-02-19) — reaches complementary conclusions from a platform strategy lens. Covers OpenAI's lack of competitive moat (no network effects, no lock-in, equivalent models from half a dozen competitors), shallow engagement data, the widget fallacy, and product strategy flowing from research rather than user needs. **Complementary coverage with Wardley Map analysis:** Evans covers OpenAI competitive weaknesses and platform dynamics; Wardley covers enterprise market share shifts and financial projections. Neither covers Apple (strengthening our point that Apple is the underexplored bet), neither covers MCP/protocol layer as a platform thesis, neither covers consumer demand-side story or sovereignty. Evans provides the strongest named counterargument to platform consolidation; our response is in the counterargument section above.

**Reminder:** Explore multi-agent architecture as a competitive dynamics question. The general orchestrator routing to specialized sub-models (smaller, domain-trained, cheaper to run) is a different economic tier than specialized services exposed via MCP. Implications: who trains the specialized models, who controls the routing, does this widen participation beyond "expose your API" to "train a model on your proprietary data." Connects to the discovery layer question — the orchestrator deciding which specialized agent to call is another form of platform power.

**Reminder from Post 3:** Develop the "MCP without DNS" thread as a competitive dynamics question. Post 1 identified the discovery layer as the key missing piece. Post 3 references it but defers the competitive question (who builds it, who captures the economics, what it looks like architecturally) to this post. This is a natural fit for the "who owns the platform services layer" argument.
...
**Reminder from Post 3:** The developer transition section (three-phase timeline, Jevons paradox, productivity multiplier) is already covered in Post 3. Post 5's "software engineer double squeeze" section should take the societal lens — job displacement, the historical irony of engineers automating themselves, the personal story — without re-deriving the mechanics. Reference Post 3 for the framework and build on it rather than restating it.
...
- **Old manipulation vectors removed, new ones introduced.** A conversational interface eliminates UI-level dark patterns (hidden fees, buried buttons, urgency messaging). But promoted placements in a trusted conversational context are the conflict-of-interest problem returning in a subtler form — the agent feels like *your* assistant, not an ad platform. The net effect is likely better than the status quo, but the new vectors are harder to detect.
...
- **The full circle has a human cost — and the conflict returns.** Human intermediaries → self-service → AI intermediaries is an elegant thesis. But each transition displaced real people and had a trade-off: human intermediaries traded conflict for convenience, self-service traded convenience for trust, AI intermediaries promise convenience again but reintroduce the conflict-of-interest in a new form. The first transition was celebrated ("technology creates efficiency"). The second one will be harder to celebrate when it's *your* industry being automated.
...
### The military question — probabilistic systems in life-and-death contexts

Post 4 establishes the Anthropic/Pentagon thread: the US government pressuring an AI company to drop safety commitments for unrestricted military access, including autonomous weapons and mass surveillance. Post 5 takes the societal lens.

- **The fundamental problem with probabilistic military AI.** Gary Marcus ([Code Red for Humanity](https://garymarcus.substack.com/p/code-red-for-humanity)) flags the core danger: LLMs are probabilistic systems that hallucinate. In simulated nuclear crisis scenarios, multiple AI models recommended nuclear escalation in approximately 95% of cases. "The nuclear taboo is no impediment to nuclear escalation by our models." Applying hallucinatory systems to weapons without humans in the loop is a categorically different risk than getting a wrong flight recommendation.
- **The dual-use problem is structural.** The same companies building the consumer platforms described in this series are being coerced into military applications. Anthropic's safety commitments — the ones that gave MCP credibility as a trustworthy protocol — can be overridden by government directive. The company that builds your personal agent and the company that builds military AI are the same company. That's not a hypothetical conflict of interest. It's the architecture.
- **Already deployed.** Marcus notes that GenAI has reportedly already been used in military operations (Maduro raids) and policy-making (tariff regulations). The question of whether AI should be used in military contexts is already moot — it's happening. The question is governance, oversight, and whether safety constraints survive political pressure.
- **The connection to the trust problem.** If we can't fully trust an agent to show unbiased consumer options (the filtered information problem from earlier in this post), the military application is the extreme case: what happens when the system that hallucinates 3% of the time is making targeting decisions? The trust gap that's merely expensive in consumer contexts is catastrophic in military ones.
- **What this means for the series thesis.** The Web 4.0 platform thesis is fundamentally about delegating decisions to AI. The military question is the reductio ad absurdum of delegation: how far does "delegate and confirm" extend? The societal answer to that question shapes what consumer-level delegation looks like too — because the governance frameworks (or lack thereof) that emerge from the military AI debate will define the boundaries for civilian AI.
